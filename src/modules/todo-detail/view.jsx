...
const InfoCard = ({ icon: Icon, label, value, subValue, color, bgcolor }) => (
    <Box sx={{ p: 2, borderRadius: 2, bgcolor, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', minWidth: 0 }}>
        ...
        <Typography variant="body2" fontWeight={700} sx={{ wordBreak: 'break-word', lineHeight: 1.2, fontSize: '0.9rem' }}>{value}</Typography>
        <Typography sx={{ color: 'text.secondary', fontWeight: 500, fontSize: '0.75rem' }}>{subValue}</Typography>
    </Box>
);
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 12 }}>
            ...
            <Paper elevation={3} sx={{ p: { xs: 3, md: 5 }, pb: { xs: 5, md: 6 }, borderRadius: 2, border: '1px solid #e2e8f0', bgcolor: 'white' }}>
                <Box display="flex" gap={3} mb={5} alignItems="flex-start" flexWrap="wrap">
                    <Avatar sx={{ width: 80, height: 80, bgcolor: 'primary.main', fontSize: '2rem', fontWeight: 800, boxShadow: '0 4px 12px rgba(59, 130, 246, 0.2)' }}>
                        {todo.title?.[0]?.toUpperCase()}
                    </Avatar>
                    <Box sx={{ flex: 1, minWidth: 0, pt: 1 }}>
                        <Chip size="small" label={isCompleted ? 'COMPLETED' : 'PENDING'} color={isCompleted ? 'success' : 'warning'} sx={{ fontWeight: 800, mb: 1, borderRadius: 1 }} />
                        <Typography variant="h4" fontWeight={800} sx={{ color: 'text.primary', width: '100%', overflowWrap: 'anywhere', wordBreak: 'break-word', lineHeight: 1.2, pb: 1 }}>
                            {todo.title}
                        </Typography>
                    </Box>
                    <Stack direction="row" spacing={1.5} sx={{ flexShrink: 0, pt: 1 }}>
                        <Button variant="contained" startIcon={<Edit2 size={18} />} onClick={onEdit} sx={{ px: 3, height: 42, textTransform: 'none' }}>Edit</Button>
                        <Button variant="outlined" color="error" startIcon={<Trash2 size={18} />} onClick={onDelete} sx={{ px: 3, height: 42, textTransform: 'none' }}>Delete</Button>
                    </Stack>
                </Box>

                <Box mb={6}>
                    <Typography variant="subtitle2" fontWeight={800} color="text.secondary" sx={{ textTransform: 'uppercase', mb: 1.5, letterSpacing: 1.5 }}>Description</Typography>
